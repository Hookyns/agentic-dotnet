...
public sealed class CodeFixesTools(WorkspaceService workspace)
{
    ...
    public async Task<string> ApplyCodeFix(
        [Description("Absolute path to the .cs file that contains the diagnostic.")]
            string filePath,
        [Description("Diagnostic ID to fix (e.g. 'CS0246').")] string diagnosticId,
        [Description("Exact title of the code fix to apply, as returned by get_code_fixes.")]
            string fixTitle,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var diagnostic = compilation
            .GetDiagnostics(cancellationToken)
            .FirstOrDefault(d => d.Location.SourceTree == tree && d.Id == diagnosticId);
