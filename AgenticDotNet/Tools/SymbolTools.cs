...
[McpServerToolType]
internal sealed class SymbolTools(WorkspaceService workspace)
{
			...
			.Select(s =>
			{
				...
				return (object)
					new
					{
						...
						Column = span.HasValue ? span.Value.StartLinePosition.Character + 1 : (int?)null,
	...
	public async Task<string> RenameSymbol(
		[Description("Absolute path to the .cs file where the symbol is declared or referenced.")] string filePath,
	...
	)
	{
		...
		try
		{
			renamedSolution = await Renamer.RenameSymbolAsync(solution, symbol, options, newName, cancellationToken);
		}
		catch (Exception ex)
		{
	...
	[Description("Find all overrides of a virtual or abstract method in derived types across the solution.")]
	public async Task<string> FindOverrides(
		[Description("Fully qualified name of the type that declares the method (e.g. 'MyApp.BaseService').")]
			string typeFullName,
	...
	private static async Task<ISymbol?> FindSymbolAtAsync(Document document, int line, int column, CancellationToken ct)
	{
